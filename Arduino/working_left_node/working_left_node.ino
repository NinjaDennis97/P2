...
WiFiClient client;
 ...
 String left_str = "j", right_str, up_str, down_str;
...
if(timeToSendUpdateLeft){
    if(leftsIP[3] != 0){
      if (client.connect(leftsIP, 5000)) {
        ...
        String temp_str ="R" + own_letter + right_str;
        int len = temp_str.length();
        char buf[len];
        temp_str.toCharArray(buf, len);
  ...
  if(timeToSendUpdateRight){
    if(rightsIP[3] != 0){
      if (client.connect(rightsIP, 5000)) {
        ...
        String temp_str = "L" + left_str + own_letter;
        int len = temp_str.length();
        char buf[len];
        temp_str.toCharArray(buf, len);
        client.write(buf, len);
  ...
  if(timeToSendUpdateUp){
    if(upsIP[3] != 0){
      if (client.connect(upsIP, 5000)) {
        ...
        String temp_str = "D" + own_letter + down_str;
        int len = temp_str.length();
        char buf[len];
        temp_str.toCharArray(buf, len);
        client.write(buf);
  ...
  if(timeToSendUpdateDown){
    if(downsIP[3] != 0){
      if (client.connect(downsIP, 5000)) {
        ...
        String temp_str = "U" + up_str + own_letter;
        int len = temp_str.length();
        char buf[len];
        temp_str.toCharArray(buf, len);
        client.write(buf);
    ...
    Serial.println("newClient");
        ...
        Serial.println("minNACK");
          ...
          if(first_char == 'L'){
            ...
            if(left_str.length() + right_str.length()+ up_str.length() + down_str.length()){
              Serial.println(left_str + own_letter + right_str);
              Serial.println(up_str +own_letter + down_str);
            }
...
void connection(){
  ...
  if (client.connect(tempIP, 5000)) {
    ...
    String response3 = client.readStringUntil('\r');
    if(response1.equals("ACK") || response2.equals("ACK") || response3.equals("ACK")){
